...
function AppShowcase({ app }: { app: App }) {
  ...
  if (app.company.includes('Signify')) {
    return (
      <div className="flex h-full flex-col items-center justify-end gap-[11cqh]">
        <div className="flex min-h-0 flex-1 items-end justify-center gap-[3cqw]">
          <DeviceMockup device="ipad" className="h-[min(76cqh,70cqw)] w-auto">
            <SignifyLogoScreen />
          </DeviceMockup>
          <DeviceMockup device="iphone" className="h-[min(49cqh,44cqw)] w-auto">
            <MySignifyLogoScreen />
          </DeviceMockup>
        </div>
        <CvsHealthLogo className="w-[min(46cqw,390px)] shrink-0" />
      </div>
    )
  }
  return (
